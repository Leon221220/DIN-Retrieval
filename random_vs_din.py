...
Final answer: A"""
def make_random_spec_like(din_spec: Dict[int, List[int]], hf_model) -> Dict[int, List[int]]:
    import numpy as np
    rng = np.random.default_rng(42)  # 或用 args.seed
    # 大多数模型各层 hidden_size 一致；若你支持异构层，可按层查询权重尺寸
    d = hf_model.config.hidden_size

    rand_spec = {}
    for L, idxs in din_spec.items():
        K = len(idxs)
        if K == 0:
            rand_spec[L] = []
            continue
        # 从 [0, d) 里不放回采样 K 个维度
        rand_spec[L] = rng.choice(d, size=K, replace=False).tolist()
    return rand_spec
...
def main():
    ...
    ap.add_argument("--randomize_din_subspace", action="store_true",
                help="用与 DIN 相同层与每层相同K的随机子空间替换 DIN")
    ...
    if args.compute_din:
        ...
        if args.randomize_din_subspace:
            vprint(f"[RANDOM] Using random selected hidden spec", verbose=args.verbose)
            din_spec = make_random_spec_like(din_spec, hf_model)
